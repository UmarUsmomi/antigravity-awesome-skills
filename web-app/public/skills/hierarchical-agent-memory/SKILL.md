...
description: "Scoped Antigravity.md memory system that reduces context token spend. Creates directory-level context files, tracks savings via dashboard, and routes agents to the right sub-context."
risk: safe
...
Scoped memory system that gives AI coding agents a cheat sheet for each directory instead of re-reading your entire project every prompt. Root Antigravity.md holds global context (~200 tokens), subdirectory Antigravity.md files hold scoped context (~250 tokens each), and a `.memory/` layer stores decisions, patterns, and an inbox for unconfirmed inferences.
...
- Use when you want directory-scoped context instead of one monolithic Antigravity.md
...
```
project/
├── Antigravity.md              # Root context (~200 tokens)
├── .memory/
...
└── src/
    ├── api/Antigravity.md      # Scoped context for api/
    ├── components/Antigravity.md
    └── lib/Antigravity.md
```

### Step 2: Context Routing

The root Antigravity.md includes a routing section that tells the agent exactly which sub-context to load:

```markdown
## Context Routing

→ api: src/api/Antigravity.md
→ components: src/components/Antigravity.md
→ lib: src/lib/Antigravity.md
```
...
- Context file health (missing/stale/inherited Antigravity.md coverage)
...
|---|---|
| `go ham` | Set up HAM — auto-detect platform, generate Antigravity.md files |
...
| `ham route` | Add/update Context Routing section in root Antigravity.md |
...
Agent: HAM setup complete. Created 8 files.
  ...
  - .memory/patterns.md
  - .memory/inbox.md
  - src/api/Antigravity.md
  - src/components/Antigravity.md
  - src/lib/Antigravity.md
  - src/utils/Antigravity.md
...
- Keep root Antigravity.md under 60 lines / 250 tokens
- Keep subdirectory Antigravity.md files under 75 lines each
...
- Dashboard requires Node.js 18+ and reads session data from `~/.Antigravity/projects/`
- Context routing detection relies on Antigravity.md read order in session JSONL files
- Does not auto-update subdirectory Antigravity.md content — you maintain those manually or via `ham audit`
